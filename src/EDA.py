...
print(epd.describe())
print(epd2.describe())
print(epd["GWPtotal (A2)"].unique())
cols = ["GWP","GWPtotal (A2)", "GWPbiogenic (A2)", "GWPfossil (A2)", "GWPluluc (A2)"]
complet= epd[cols].notna().mean() * 100 
plt.figure(figsize=(8, 6))
complet.plot(kind="bar")
plt.ylabel("Porcentaje de valores disponibles")
plt.xticks(rotation=45, ha="right")
plt.title("Disponibilidad de indicadores GWP ")
plt.ylim(0, 110)
plt.tight_layout()
plt.savefig(IMG_DIR/"Disponibilidad de indicadores GWP.png")
plt.show()
#Como se distribuye la variables GWP (A2)
colum = ["GWPtotal (A2)", "GWPbiogenic (A2)", "GWPfossil (A2)", "GWPluluc (A2)"] 
plt.figure(figsize=(7, 4))
for col in colum:
    plt.plot(epd[col], "o", label=col, alpha=0.6)
plt.axhline(0, linestyle="--", linewidth=1)
plt.ylabel("Valor GWP (A2)")
plt.title("Comparación entre indicadores GWP por módulo")
plt.legend()
plt.grid(alpha=0.3)
plt.savefig(IMG_DIR/"Comparación entre indicadores GWP por módulo.png")
plt.show()


# seleccionar columnas relevantes y renombrar
epd_subset = epd[['Name (en)', 'Ref. unit', 'Bulk Density (kg/m3)', 'GWPfossil (A2)', 'Module']].copy()
epd_subset = epd_subset.rename(columns={'GWPfossil (A2)': 'GWP'})
